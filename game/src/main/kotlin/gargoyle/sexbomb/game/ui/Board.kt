...
import gargoyle.sexbomb.game.beans.*
...
class Board(private val applet: Applet, game: Game, resBgTitle: URL) : JComponent(), GameListener {
    ...
    init {
        if (!::bgTitle.isInitialized) {
            bgTitle = resBgTitle.let { applet.getImage(it) }
        }
        this.game = game
        ...
        if (game.isGame) {
            ...
            if (isVisible) {
                if (prev == null) {
                    ...
                    val localHighlight = highlight!!
                    if (prev.x != localHighlight.x || prev.y != localHighlight.y) {
                        repaint()
                    }
                }
            }
    ...
    override fun paint(g: Graphics) {
        if (!game.fieldInitialized() || game.isInit) {
            super.paint(g)
            paintTitle(g)
            return
        }
    ...
    private fun paintCell(g: Graphics, x: Int, y: Int) {
        ...
        if (game.skinInitialized()) {
            ...
            if (game.isLost) {
                ...
                } else if (!cell.isOpen && cell.flag != null) {
                    if (cell.isMine) {
    ...
    private fun paintCellBorder(g: Graphics, x: Int, y: Int, cell: Cell, rectangle: Rectangle) {
        val c = g.color
        g.color = if (highlight != null) {
            val localHighlight = highlight!!
            if (x == localHighlight.x && y == localHighlight.y) Color.YELLOW else Color.BLACK
        } else Color.BLACK
