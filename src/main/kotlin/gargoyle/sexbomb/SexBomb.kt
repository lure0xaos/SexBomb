...
class SexBomb : GXApplet(
    ...
    private fun createMenuBar(configUrl: URL?, charset: Charset): JMenuBar {
    ...
    private fun createMenuCampaign(configUrl: URL?, charset: Charset): JMenu {
        val menu = JMenu(Campaign::class.java.simpleName)
        try {
            InputStreamReader(configUrl!!.openStream(), charset).use { reader ->
                val file = PropertyMap(reader)
                val entries = file.entrySet()
                for ((key, value) in entries) {
                    if (key.startsWith(PREFIX_CAMPAIGN)) {
                        val url = Res.url(configUrl, value)
                        if (url != null) {
                            val campaign = Campaign(url)
                            val item = JMenuItem(campaign.name)
                            item.addActionListener { newGame(campaign) }
                            menu.add(item)
                        }
                    }
                }
            ...
            Log.error(e1.localizedMessage, e1)
        }
        return menu
    }

    private fun createMenuEdit(configUrl: URL?, charset: Charset): JMenu {
    ...
    override fun doInit() {
        ...
        game = Game()
        board = Board(game, Help.getTitleUrl())
        ...
        val configUrl =
            Res.nearClassUrl(SexBomb::class.java, String.format("%s.properties", SexBomb::class.java.simpleName))
        val menuBar = createMenuBar(configUrl, Charset.defaultCharset())
    ...
    private fun createMenuFile(configUrl: URL?, charset: Charset): JMenu {
    ...
    private fun createMenuLaF(): JMenu {
        ...
        for (laf in UIManager.getInstalledLookAndFeels()) {
            val item = JMenuItem(object : AbstractAction(laf.name) {
                override fun actionPerformed(e: ActionEvent) {
                    changeLookAndFeel(laf.className)
                }
            })
            menu.add(item)
        }
    ...
    private fun createMenuSkin(configUrl: URL?, charset: Charset): JMenu {
        ...
        try {
            InputStreamReader(configUrl!!.openStream(), charset).use { reader ->
                ...
                val entries = file.entrySet()
                for ((key, value) in entries) {
                    if (key.startsWith(PREFIX_SKIN)) {
                        val url = Res.url(configUrl, value)
                        if (url != null) {
                            val skin = Skin(url, charset)
                            val item = JMenuItem(object : AbstractAction(skin.name) {
                                override fun actionPerformed(e: ActionEvent) {
                                    setSkin(skin)
                                }
                            })
    ...
    override fun doStop() {}
    private fun newGame(campaign: Campaign) {
        game.start(campaign)
    }
